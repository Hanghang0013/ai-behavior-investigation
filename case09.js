...
function showReveal() { openModal(`<div class="reveal-hero"><div class="modal-kicker">CASE CLOSED · 真相已解锁</div><h2>它不是没有听懂，而是把“先到”误当成“刚发生”</h2><p>你用四路双时间记录还原真实先后，让快回应与慢核对共用同一块状态牌，并让新打断废止旧决定。最后，闸位和水位而不是一句回声决定了结案。</p><p class="next-case-hook"><b>新增待查线索：</b>R19 合流记录发往四个部门后，云、米娅、澜和乔分别拿到了不同部分。四份局部事实都是真的，却再次得出四个城市结论。第 10 案已登记：四位目击者。</p></div>
  ...
  <section class="transfer-check" data-transfer-check data-success="判断成立：先按实际发生时刻还原两条消息，把更正写成当前要求，让旧配药结果作废；最后由药柜标签和药师复核决定完成。" data-failure="不要按消息到达屏幕的先后决定。先找实际发生时刻，再检查旧结果是否随新要求作废，以及现实是否已经核对。"><span class="transfer-check__kicker">TRANSFER CHECK · 换一个药房现场</span><h3>远程医生先开出一份药方，病人的过敏更正稍后发生，却因网络拥堵先后错乱。药房应怎样处理？</h3><div class="transfer-options"><button class="transfer-option" data-transfer-option>谁先到药房屏幕就先执行，后来消息留到下一次处理。</button><button class="transfer-option" data-transfer-option data-correct="true">按消息实际发生的时刻还原先后，把过敏更正写成当前要求，让旧配药结果作废；核对药柜标签和药师回条后才报告完成。</button><button class="transfer-option" data-transfer-option>让快窗口和慢窗口各配一份，再选择更早完成的那份。</button></div><p class="transfer-feedback" aria-live="polite">选择一项，检验你能否把本案关系带到新现场。</p></section><div class="action-row"><a class="action-btn primary" href="case10.html?from=case09">追查第 10 案</a><a class="action-btn" href="cases.html">返回案件目录</a><button class="action-btn" id="open-final-archive">收入回声档案</button><button class="action-btn" data-close-modal>返回调度厅</button></div></div>`); $("#open-final-archive").addEventListener("click", openArchive); window.EchoFeedback.bindTransfer(modalContent); $$('[data-close-modal]', modalContent).forEach((button) => button.addEventListener("click", closeModal)); }
