...
function showReveal() { openModal(`<div class="reveal-hero"><div class="modal-kicker">CASE CLOSED · 第一卷终章</div><h2>四位目击者没有说谎，错的是把局部真话当成了完整答案</h2><p>你没有让四个人重复同一份摘要，而是保留现场、进度、历史和执行四种独立证据；共同任务牌只共享行动所需内容，危险钥匙留在明确边界内，最后由城市现实决定完成。</p><p class="next-case-hook"><b>回声网络修复记录：</b>从第一个错误的“已经送达”，到四位目击者共同证明的城市现实结果，十处节点重新亮起。调查局第一卷正式归档。</p></div>
  ...
  <div class="action-row"><a class="action-btn primary" href="cases.html">返回十案目录</a><a class="action-btn" href="index.html">返回主页</a><button class="action-btn" id="open-final-archive">收入回声档案</button><button class="action-btn" data-close-modal>返回联合调查厅</button></div></div>`); $("#open-final-archive").addEventListener("click", openArchive); $$('[data-close-modal]', modalContent).forEach((button) => button.addEventListener("click", closeModal)); }
